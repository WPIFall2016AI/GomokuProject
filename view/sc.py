'''
This is the file for the view part
'''

import pygame
from global_var import global_var as gv

def load_img(file_loc, size = (0,0)):
...
class Button():
    def __init__(self, btn_file_loc = (None, None, None), size = (0,0), position = (0,0)):
        ...
        self.static_img = load_img(btn_file_loc[0], size)
        self.focus_img = load_img(btn_file_loc[1], size)
        self.down_img = load_img(btn_file_loc[2], size)

    # judge whether the mouse pointer is in the button area
...
class Anot_Button():
    def __init__(self, title, btn_file_loc = (None, None, None), size=(0,0), position=(0,0)):
        self.screen = gv.g_screen
        self.position = position
        self.button_pushed = 0
        self.border_x = (position[0], position[0]+size[0])
...
class ProgressBar():
    def __init__(self, file_loc, size, pos, len):
        self.screen = gv.g_screen
        self.img = load_img(file_loc, size)
...
def draw_table(core, w_img, b_img):
    ...
    # From index 1, draw the move number
